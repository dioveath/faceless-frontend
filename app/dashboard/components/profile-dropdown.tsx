...
export function ProfileDropdown({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  if (loading) {
    return (
      <div className={cn("flex items-center space-x-2", isCollapsed && "flex-col space-x-0 space-y-2")}>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button variant="ghost" className={cn("w-full flex flex-1 items-center py-6", isCollapsed ? "w-full p-0 justify-center" : "w-full justify-start")}>
      ...
      <DropdownMenuContent className="w-64 bg-popover text-popover-foreground">
        ...
        <div className="p-4 pt-0">
          <Link href="/dashboard/subscription" passHref>
            <Button variant="outline" className="w-full text-foreground border-border hover:bg-accent hover:text-accent-foreground">
              Upgrade Plan
            </Button>
          </Link>
        </div>
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <DropdownMenuItem className="hover:bg-accent hover:text-accent-foreground" disabled={loading} onSelect={(e) => e.preventDefault()}>
